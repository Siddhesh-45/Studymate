...
function buildSchedule(pendingTasks, weeklyHours, sessionDuration, startDate) {
  ...
  while (tasks.length > 0 && dayOffset < MAX_DAYS) {
    ...
    while (tasks.length > 0 && hoursLeft > 0.1) {
      ...
      if (needed <= hoursLeft || dayTasks.length === 0) {
        // Entire task fits OR it's the only task we can fit today
        ...
        hoursLeft = Math.max(0, parseFloat((hoursLeft - needed).toFixed(2)));
        tasks.shift();
      } else {
        // Can't fit today, and we already have a task. Skip to next day.
        hoursLeft = 0;
      }
    }
...
exports.getSmartSchedule = async (req, res) => {
  try {
    ...
    res.json({ schedule, tasksByDate, autoGenerated: false });
  } catch (err) {
    ...
    if (status === 'missed') {
      ...
      for (let d = foundDayIdx + 1; d < schedule.days.length; d++) {
        ...
        if (remaining >= 0.5) {
          ...
          const nextDateStr = new Date(targetDay.date).toISOString().split('T')[0];
